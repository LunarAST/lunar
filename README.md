...
| `lunar map` | Generate ecosystem topology. With `--config`, use a config file. Without config, auto-detects all projects in `LUNAR_PROJECTS_DIR` (default `/opt/`). Supports `--upload` and `--output`. |
